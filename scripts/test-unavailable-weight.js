...
async function checkUnavailableShiftsWholeRampDownAndIsRetappable() {
  ...
  // authored at 5 sets, so the two-weight ramp is [170, 170, 185, 185, 185]
  // (CHANGES-2026-09-13, Phase 1).
  const before = weights(card);
  console.log("Leg Press ramp before unavailable:", JSON.stringify(before));
  if (JSON.stringify(before) !== JSON.stringify(["170", "170", "185", "185", "185"])) {
  ...
  // around the new target: [155, 155, 170, 170, 170].
  if (JSON.stringify(afterOne) !== JSON.stringify(["155", "155", "170", "170", "170"])) {
  ...
  if (JSON.stringify(afterTwo) !== JSON.stringify(["140", "140", "155", "155", "155"])) {
