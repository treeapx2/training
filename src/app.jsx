...
// Ramp shape scales with `setCount` (see deriveSetCount) instead of always
// spending a fixed warmup+build overhead — the old fixed 5-slot shape was
// wrong for short movements (see CHANGES.md Aug 19 2026, Phase 2: "no
// warmups for a three set exercise", confirmed by real logs — Skull
// Crusher ran 15x10 four times, Hammer Curl ran 15x10 three times, neither
// with a warmup). Table, for target T and step i:
//   5 sets: [T-2i, T-i, T-i, T, T]  (unchanged — warmup, 2 build, 2 working)
//   4 sets: [T-i, T, T, T]          (1 build, no warmup)
//   3 sets: [T, T, T]               (no warmup, no build)
//   2 sets: [T, T]
//   1 set (floor, not in the table but never go below it): [T]
// setCount > 5 pads more working sets at T onto the full 5-slot shape.
