...
// ── Block State ───────────────────────────────────────────────────────────────
const BLOCK = {
  // Authored in the training project and handed in whole (see CLAUDE.md
  // "Scope boundary"). Replaced entirely Sep 8 2026: the blood-thinner
  // precaution and the hockey suspension are both retired (PE workup came
  // back clear), the Legs A/B flag went with the variants, and the schedule /
  // time budget / volume target / superset placement are now written down
  // here rather than living only in the work order.
  flags: [
    "Hockey is BACK \u2014 1-2x/week on unpredictable days since 8/24. PE workup came back clear. First session back: best speed and stamina in months.",
    "SCHEDULE: 4 gym days/week, doubling either Push or Pull. Legs 1x/week \u2014 skating supplies additional lower-body load. Hockey 1-2x, 1-2 rest/travel days.",
    "TIME: 45 min lifting, then a separate ~15 min cardio finisher. Rests: opener 150s, movement 2 120s, movements 3-4 90s, superset 60s after each pair.",
    "VOLUME TARGET: ~6-8 working sets per muscle group (supersedes the old 100-reps-per-group target). 3 muscle groups x 2 movements per session.",
    "ONE superset per session, always in the final slots, to finish a muscle group under fatigue. Sub-maximal output there is intended \u2014 it does not count against progression.",
    "PROGRESSION VALIDITY: run the lift you want to advance FIRST. Confirmed repeatedly \u2014 Shoulder Press 90 to 75, Cable Curl 52.5 to 47.5, Rope Pushdown 42.5 to 37.5 all came from queue position, not lost strength.",
    "LEG PRESS: 185 has been RPE 8 for six sessions. The limiter is unracking off the hinges, not the working reps \u2014 do NOT add weight. Use a 3-second eccentric at 185 instead, or check whether the carriage start height is adjustable.",
    "Left knee below patella \u2014 monitor squat/hinge, bilateral leg press only. Tightness on 8/10 traced to neglected mobility: 5 min quad/hip-flexor stretch + foam roll before legs.",
    "Groin asymmetry (L>R) \u2014 daily butterfly + lateral lunge holds. Extra relevant with hockey back and tennis in rotation.",
    "Thumb \u2014 watch on Shoulder Press. Elbow \u2014 Skull Crusher pull/stretch is normal, sharp pain is a stop signal.",
    "Dumbbell rack: 5 lb increments to 50, plus a 12 lb pair. No other weights available.",
    "Chest: DB Bench Press preferred over the Chest Press machine. Both remain available."
  ],
  sessions: {
    legs: {
      label: "Legs",
      color: "#3B6D11",
      bg: "#EAF3DE",
      rest: "M1 60/60/150/150s \u00b7 M2 60/120/120s \u00b7 M3 45/90/90s \u00b7 superset 60s after each pair",
      movements: [
    ...
    push: {
      ...
      color: "#185FA5",
      bg: "#E6F1FB",
      rest: "M1 60/60/150/150s \u00b7 M2 60/120/120s \u00b7 M3-4 45/90/90s \u00b7 superset 60s after each pair",
      movements: [
        { name: "DB Bench Press", current: "35 lb", steps: DUMBBELL_STEPS, reps: 10, target: "Build clean 10/10 at 35 per hand \u2192 then 40. Logged 30s\u00d710\u00d73 @ RPE 5-6 and 35s\u00d710\u00d72 @ RPE 7 (9/5). Currently the preferred chest movement over the Chest Press machine." },
        { name: "Pec Fly", current: "120 lb", increment: 15, reps: 10, target: "TEST 135 \u2014 120x10x2 @ RPE 7 (7/28), room to spare." },
        { name: "Rope Pushdown", current: "42.5 lb", increment: 5, reps: 10, target: "Confirm 2nd clean 10/10 @ 42.5 then 47.5. Skipped 7/28 (machine in use)." },
        ...
        { name: "Shoulder Press", current: "90 lb", increment: 15, reps: 10, target: "HOLD 90 \u2014 needs a FRESH lead to confirm. Hit 90x10x2 when run first (7/5), dropped to 75x8 @ RPE 8 when run second (7/28). Confirm then 105. Thumb watch. Superset with Lateral Raise." },
        { name: "Lateral Raise", current: "15 lb", steps: DUMBBELL_STEPS, reps: 12, target: "Back to 15 \u2014 12x10x4 reached RPE 8 at position #4 (7/28). 15x10x2 already done 7/5. Superset with Shoulder Press." }
      ]
    },
    pull: {
      label: "Pull",
      ...
      movements: [
        ...
        { name: "DB Row", current: "50 lb", steps: DUMBBELL_STEPS, reps: 10, target: "Build 8-10 @ 50 \u2014 45x10 @ RPE 7 as fresh opener (7/25), best DB Row log to date. Fatigue-sensitive." },
        ...
        { name: "Cable Curl", current: "42.5 lb", increment: 5, reps: 10, target: "TEST 47.5 \u2014 two clean sessions at 42.5 (7/4, and 7/25 4x10 @ RPE 6-7). Superset with Hammer Curl." },
        { name: "Hammer Curl", current: "20 lb", steps: DUMBBELL_STEPS, reps: 10, target: "HOLD 20 \u2014 read heavy cold (7/25: 20x8 @ RPE 8, dropped to 15). TEST 25 only on a day this LEADS the session. Superset with Cable Curl." }
      ]
    }
  }
};
...
// ── Movement metadata ──────────────────────────────────────────────────────────
// Muscle group definitions — three groups per session type, two default
// movements each, matching the session architecture in CHANGES.md Sep 8 2026,
// Phase 3. Optional-add movements (Phase 4) are listed alongside the defaults
// in whichever group they belong to, so a movement added to a session still
// gets a real group chip instead of falling through to "Other".
//
// The group labels themselves come from Phase 3's tables (Quads / Posterior /
// Calves, Chest / Triceps / Shoulders, Back / Upper back / Biceps), replacing
// the older Quad / Hamstring / Glute / Calf and Bicep / Rear Delt splits —
// Goblet Squat in particular moves from "Glute" to "Posterior" there.
const MUSCLE_GROUPS = {
  legs: [
    {
      label: "Quads",
    ...
    {
      label: "Posterior",
      movements: ["Leg Curl", "Goblet Squat", "RDL", "Glute Bridge"],
    },
    {
      label: "Calves",
  ...
  push: [
    ...
    {
      label: "Triceps",
      movements: ["Rope Pushdown", "Skull Crusher", "OHE"],
    },
    {
      label: "Shoulders",
      movements: ["Shoulder Press", "Lateral Raise", "Shoulder Press (DB)"],
    },
  ],
  pull: [
    {
      label: "Back",
      movements: ["Seated Row", "Lat Pulldown"],
    },
    {
      label: "Upper back",
      movements: ["DB Row", "Reverse Fly", "Rows"],
    },
    {
      label: "Biceps",
      movements: ["Cable Curl", "Hammer Curl", "Zottman Curl"],
    },
  ],
};
...
//
// Revised again Sep 8 2026 (CHANGES.md Phase 3) to EXACTLY ONE pre-seeded
// pair per session type, always in the final two slots — the superset is now
// the third muscle group of a three-group session, placed last by design so
// that group gets finished under fatigue (which is also why Phase 2 excludes
// superset sessions from baseline derivation). Rope Pushdown <-> Skull
// Crusher and Cable Curl <-> Reverse Fly are gone; Legs gains its first
// pre-seeded pair. Manual linking of any adjacent pair stays available.
//
// The Aug 19 revision's reasoning still holds for what's here: no
// machine+machine pairs ("no supersets with machines out of respect for the
// gym goers"), and Shoulder Press <-> Lateral Raise remains the one
// machine-inclusive pair the owner explicitly endorsed.
...
function buildHandoff(history) {
  ...
  // Conditioning/injury status is authored in BLOCK.flags and printed below
  // under ACTIVE FLAGS. A hardcoded status line here duplicated it and then
  // went stale — it was still telling the coach hockey was suspended for a
  // blood-thinner precaution weeks after both were retired.
  lines.push(
...
function movementSessionSummaries(history, movName, targetReps, opts) {
  const excludeSupersets = !!(opts && opts.excludeSupersets);
  const out = [];
  history.forEach((session) => {
    const mov = session.movements.find((m) => m.name === movName);
    ...
    // Superset context (see CHANGES.md Sep 8 2026, Phase 2): sets performed
    // as half of a superset are deliberately sub-maximal — the owner matches
    // weights across a free-weight pair to avoid rack trips, and places
    // supersets last precisely to finish a muscle group under fatigue. Those
    // sessions must not set (or lower) the movement's own baseline.
    const inSuperset = !!mov.supersetId;
    if (excludeSupersets && inSuperset) return;
...
// The sessions that may legitimately drive progression for a movement, most
// recent first (see CHANGES.md Sep 8 2026, Phase 2). Superset sessions are
// dropped first; only a movement with NO non-superset history at all falls
// back to counting them.
//
// DEVIATION from the work order, flagged deliberately: Phase 2 says a
// superset-only movement should "fall back to the best non-superset session,
// then to BLOCK.current". Taken literally that means BLOCK.current — there is
// no non-superset session to fall back to — which would freeze every
// permanent superset member (after Phase 3 that is Shoulder Press, Lateral
// Raise, Cable Curl, Hammer Curl, Goblet Squat, Calf Raise) at a
// hand-authored string forever. That is exactly the staleness the target
// picker was built to remove, and it would defeat this phase's own goal. So
// superset history is used when it is the only history there is; the
// "must not lower its own baseline" guarantee still holds, because
// deriveCurrentWeight takes the BEST qualifying session in the window, never
// the most recent one. Raise this with the owner if the frozen-baseline
// reading was actually intended.
function progressionSummaries(history, movName, targetReps) {
  const solo = movementSessionSummaries(history, movName, targetReps, {
    excludeSupersets: true,
  });
  if (solo.length) return solo;
  return movementSessionSummaries(history, movName, targetReps);
}

// The "hold" chip's weight — the heaviest weight that reached the rep range's
// lower bound across the last THREE eligible sessions containing this
// movement, not whatever the most recent session happened to land on
// (CHANGES.md Sep 8 2026, Phase 2). Reading only the most recent session let
// a single dip reset the baseline: a weight-matched superset round, a late
// placement, a rack substitution, or just a bad day. Real case from the log —
// Hammer Curl's best is 25 (Aug 9, 10 reps @ RPE 7), but Aug 19 was a
// weight-matched superset round at 15, so the engine derived 15 and its
// repeated `suggested: up` was pointing at 20 instead of 30. The owner was not
// disagreeing with the suggestion; the baseline under it was wrong.
//
// Superset sessions are excluded by progressionSummaries. Taking the max
// across the window (rather than the most recent entry) is also what makes
// the "a superset round must never LOWER the baseline" guarantee hold in the
// fallback case where superset sessions are all there is.
//
// Falls back to BLOCK.current only when the movement has never been logged.
function deriveCurrentWeight(history, mov) {
  const window = progressionSummaries(history, mov.name, mov.reps).slice(0, 3);
  if (!window.length) return parseFloat(parseCurrentWeight(mov.current)) || 0;
  // "Qualifying" = reached the rep range's lower bound. If nothing in the
  // window did, fall back to the most recent session's top weight rather
  // than the window's max — a failed heavy attempt should not become the new
  // baseline just for being the heaviest number present.
  const qualifying = window.filter((s) => s.hitFloor);
  if (!qualifying.length) return window[0].topWeight;
  return Math.max(...qualifying.map((s) => s.topWeight));
}
...
function deriveSetCount(history, movName) {
  const summaries = progressionSummaries(history, movName, 0).slice(0, 3);
...
//
// Sep 8 2026 (CHANGES.md Phase 2) added the plateau break (isPlateaued,
// below) and moved the history read onto progressionSummaries, so superset
// rounds no longer feed the suggestion either. Rules, in evaluation order:
//   last session's (first) top-weight set at RPE >= 9        -> down
//   three consecutive sessions parked at target reps, RPE <=8 -> up
//   missed the floor, fresh jump, target-4..target-2, RPE <=8 -> hold
//   missed the floor otherwise                                -> down
//   both of the last two at exact target reps, RPE <= 7       -> up
//   reached the floor but neither up condition                -> hold
//   fewer than 2 eligible sessions                            -> hold
function suggestChip(history, movName, targetReps) {
  const summaries = progressionSummaries(history, movName, targetReps);
  if (summaries.length < 2) return "hold";
  const [last, prev] = summaries;
  if (last.topRpe >= 9) return "down";
  if (isPlateaued(summaries)) return "up";
  if (!last.hitFloor) {
...
// The RPE-8 plateau trap (see CHANGES.md Sep 8 2026, Phase 2). "Target reps
// at RPE 8" satisfies neither the up rule (which wants RPE <= 7 twice running)
// nor any down rule, so a lift parked there could never be suggested up: Seated
// Row and Lat Pulldown both sat at 135x10 for seven sessions, including
// position-1 attempts, so placement wasn't the cause. Three or more consecutive
// sessions like that is consolidation, not a ceiling.
//
// INTERPRETED BROADER THAN WRITTEN, flagged deliberately: the work order says
// "target reps at RPE 8 for three or more consecutive sessions". Read strictly
// as RPE exactly 8, the rule does not fire for either movement it was written
// to fix — Seated Row's and Lat Pulldown's last three sessions are 8 / 8 / 7,
// not 8 / 8 / 8. So the test here is target reps at RPE <= 8 (i.e. not
// failing), at the SAME top weight, for the three most recent eligible
// sessions. The same-weight requirement is what keeps this from colliding with
// the Aug 19 consolidation rule, which deliberately says "hold" for a FRESH
// weight jump landing at RPE <= 8 — parked at one weight and just arrived at a
// new one are opposite situations. Tell the owner if the literal
// exactly-RPE-8 reading was meant instead.
function isPlateaued(summaries) {
  if (summaries.length < 3) return false;
  const window = summaries.slice(0, 3);
  const weight = window[0].topWeight;
  return window.every(
    (s) => s.topWeight === weight && s.hitTarget && s.topRpe > 0 && s.topRpe <= 8,
  );
}
...
// Ramp shape scales with the movement's QUEUE POSITION, not with its
// historical set count (see CLAUDE.md "Target picker" — CHANGES.md Sep 8
// 2026, Phase 1, which replaces the Aug 19 set-count table outright).
//
// The correcting insight: warm-up is a per-session need, not a per-movement
// need. Quads don't need re-warming three times inside one legs session, so
// only the opener earns a full ramp. The old shape put just 2 of 5 sets at
// working weight — 60% warm-up tax — against a target of ~20 working sets
// per session inside a 45-minute lifting budget.
//
// Table, for target T and step i (one chip-step down):
//   position 1 (opener): [T-2i, T-i, T, T, T]  5 sets, 3 working
//   position 2+:         [T-i, T, T, T]        4 sets, 3 working
//   superset member:     [T, T, T, T]          4 sets, 4 working
//   2-set movement:      [T, T]                2 sets, 2 working
//
// `opts` is { position, isSuperset, setCount }:
//   - `position` is the movement's index in the session's flat ordered list,
//     so the opener is position 0.
//   - `isSuperset` comes straight from the movement's `supersetId`. Superset
//     members never carry warm-up sets — they sit at the end of a session by
//     design and the muscle group is already fully warm — and they always get
//     exactly 4 rounds regardless of derived set count, since SupersetRow
//     interleaves the pair's rows and a mismatched count renders ragged.
//   - `setCount` (from deriveSetCount) is now consulted for ONE case only:
//     the table's "2-set movements" row. INFERENCE — the work order lists that
//     row without saying what makes a movement a 2-set movement; reading it as
//     "history says this movement is done in 2 sets" is the only reading that
//     leaves the row with any work to do, so a derived count of <=2 overrides
//     the positional shape (with a 1-set floor, never zero). Flag to the owner
//     if a 2-set movement was meant to be an explicit per-movement setting
//     instead.
// Note there is no longer any padding beyond the tabulated counts: position
// determines the set count now, which is the whole point of the restructure
// (capping total sets against the time budget). "+ add round" / handleUpdate
// ("_add") is still how extra sets get added by hand.
function buildRamp(mov, targetWeight, opts) {
  const o = opts || {};
  const position = o.position || 0;
  const isSuperset = !!o.isSuperset;
  const setCount = o.setCount;
  const wu = stepWeight(mov, targetWeight, -2);
  const b = stepWeight(mov, targetWeight, -1);
  const T = targetWeight;
  const W = { weight: T, type: "W" };
  let shape;
  if (isSuperset) {
    shape = [W, W, W, W];
  } else if (setCount != null && setCount <= 2) {
    shape = setCount <= 1 ? [W] : [W, W];
  } else if (position === 0) {
    shape = [{ weight: wu, type: "WU" }, { weight: b, type: "B" }, W, W, W];
  } else {
    shape = [{ weight: b, type: "B" }, W, W, W];
  }
  return shape.map((s) => ({
...
function useMovementPicker(mov, history, position, total, onChange) {
  ...
  // Superset membership drives the ramp shape (see buildRamp — superset
  // members get 4 working sets and never a warmup). Read straight off the
  // movement, so linking/unlinking mid-session is picked up by the
  // regeneration effect below without threading another prop through.
  const isSuperset = !!mov.supersetId;
  const rampOpts = () => ({
    position,
    isSuperset,
    setCount: deriveSetCount(history, mov.name),
  });
  ...
  const applyTarget = (choice, newWeight) => {
    const ramp = buildRamp(mov, newWeight, rampOpts());
  ...
  // Reordering a movement changes its ramp shape (see buildRamp), so the
  // ramp regenerates when its position — or its superset membership —
  // changes (CHANGES.md Sep 8 2026, Phase 1). Before any set is logged this
  // is silent; once sets are logged it goes through the same
  // confirm-before-change pattern a chip switch uses, and cancelling keeps
  // the existing ramp (the movement still moved; only its shape is left
  // alone). The ref seeds itself on the first render so a freshly-mounted
  // movement never regenerates just for existing.
  const rampShapeKeyRef = useRef(null);
  useEffect(() => {
    const key = position + "|" + (isSuperset ? "1" : "0");
    ...
    if (done) {
      const ok = window.confirm(
        "This movement moved, which changes today's ramp shape. Logged sets are kept, not discarded. Regenerate?",
      );
      if (!ok) return;
    }
