...
// Superset sessions and the baseline (CHANGES-2026-09-13, Phase 4).
//
// Sep 8 excluded superset sessions from progression outright, whenever ANY
// solo history existed. The intent was directional — "a superset round must
// never LOWER the baseline", because the owner weight-matches across a pair
// and runs it last under fatigue, so that output is deliberately sub-maximal.
// Implementing it as wholesale invisibility went too far: a superset round
// could no longer RAISE the baseline either, so a movement that lives
// permanently inside a pair got judged on stale solo data forever.
//
// That is the Calf Raise bug. It has been in the Goblet Squat superset since
// Aug 10, so its best and cleanest work — 45x15x3 @ RPE 6 (Aug 25) and
// 45x15x4 @ RPE 6-7 (Sep 3) — was discarded, and the baseline fell back to
// Jul 29's 40x20 with a `down` suggestion on top. Goblet Squat escaped the
// same fate only by luck: its stale solo sessions happen to sit at 50, its
// best. So the symptom was movement-specific while the cause was not.
//
// NOT the cause, despite the reasonable suspicion: rep-range bounds. Calf
// Raise's `reps: 15` is a single lower bound, not a 15-20 range, and
// `hitFloor`/`hitTarget` already score with >=. Both Aug 25 and Sep 3 come
// through as clean passes; they were simply thrown away before scoring
// mattered.
//
// The guarantee is now enforced by DIRECTION rather than by exclusion:
//   - deriveCurrentWeight takes the MAX qualifying weight in the window, so a
//     weight-matched superset round can only ever raise it.
//   - suggestChip refuses to return `down` off a superset session (see the
//     guard there), so sub-maximal output still can't read as regression.
// Both keep the Sep 8 fixtures green while letting real superset work count.
//
// This also retires the flagged Sep 8 deviation about superset-only movements
// falling back to `BLOCK.current`: there is no solo-preference left to fall
// back from, so the question no longer arises.
...
//
// Taking the MAX across the window (rather than the most recent entry) is
// what enforces "a superset round must never LOWER the baseline" — see the
// note above deriveCurrentWeight on why that replaced excluding those
// sessions outright.
//
// Falls back to BLOCK.current only when the movement has never been logged.
// The best weight a window of sessions supports: the heaviest that reached the
// rep range's lower bound, or — if none did — the most recent top weight,
// rather than the window's heaviest failed attempt.
function baselineFrom(window) {
...
function deriveCurrentWeight(history, mov) {
  const all = movementSessionSummaries(history, mov.name, mov.reps);
  if (!all.length) return parseFloat(parseCurrentWeight(mov.current)) || 0;
  // Two windows, three sessions each, scored separately — which is what makes
  // "never lower, may raise" hold. A straight max over the last three sessions
  // of any kind would NOT: a movement that has been in a superset for its last
  // three outings would have its solo history pushed out of the window
  // entirely, which is how Hammer Curl derived 15 against a best of 25 before
  // Sep 8, and how it started doing so again when this was first simplified.
  const solo = all.filter((s) => !s.inSuperset).slice(0, 3);
  const superset = all.filter((s) => s.inSuperset).slice(0, 3);
  const soloBase = baselineFrom(solo);
  // Superset rounds contribute only their best QUALIFYING weight, and never
  // the "nothing qualified" fallback — they may raise the baseline, never set
  // it downward.
  const ssQualifying = superset.filter((s) => s.hitFloor).map((s) => s.topWeight);
  const ssBest = ssQualifying.length ? Math.max(...ssQualifying) : null;
  if (soloBase == null && ssBest == null) return all[0].topWeight;
  if (soloBase == null) return ssBest;
  if (ssBest == null) return soloBase;
  return Math.max(soloBase, ssBest);
}

// Today's total set count, authored per movement (CHANGES-2026-09-13, Phase 1).
//
// This REPLACES the history-derived `deriveSetCount`. Set counts are now a
// programming decision rather than something inferred from what happened to
// get logged: "Set counts are per movement, not purely positional. Default to
// 5 and reduce where timing requires it." The authored numbers are calibrated
// against real timing — Sep 8 push ran 5,4,4,4,4,4 = 25 sets in 43 minutes,
// inside the 45-minute lifting budget, and Legs gets 5,5,4,4,4 because it has
// roughly ten minutes of headroom against that budget.
//
// A movement with no authored `sets` (an optional add from the movement
// library, or one the owner defined in-app) falls back to DEFAULT_SET_COUNT.
const DEFAULT_SET_COUNT = 5;
function setCountFor(mov) {
  const n = mov && mov.sets;
  return typeof n === "number" && n > 0 ? n : DEFAULT_SET_COUNT;
}
...
// below). Sep 13 2026 (Phase 4) then replaced the blanket superset exclusion
// with the directional `notDown` guard. Rules, in evaluation order:
...
function suggestChip(history, movName, targetReps) {
  ...
  // A superset session must never produce a `down` (CHANGES-2026-09-13,
  // Phase 4). Weight-matched output run last under fatigue is sub-maximal by
  // design, so reading it as regression is exactly the mistake Sep 8 set out
  // to prevent — this is that guarantee stated directionally, rather than by
  // hiding the session entirely and losing its upside too.
  const notDown = (call) => (call === "down" && last.inSuperset ? "hold" : call);
  if (last.topRpe >= 9) return notDown("down");
...
// TWO weights per exercise, never three (CHANGES-2026-09-13, Phase 1). This
// replaces the Sep 8 position-aware ramp outright, which is itself the second
// replacement of this logic — see CLAUDE.md "Target picker" for the lineage.
//
...
// weight — which is why they're typed "E" rather than the old "WU"/"B", and why
// the count of them is small and fixed rather than scaled by queue position.
//
// For target T and one increment down E:
//
//   5 sets: [E, E, T, T, T]   2 established, 3 at target
//   4 sets: [E, T, T, T]      1 established, 3 at target
//   3 sets: [T, T, T]         0 established, 3 at target
//   2 sets: [T, T]            0 established, 2 at target
//
// Generalised as "2 established at >=5 sets, 1 at exactly 4, 0 below that", so
// a movement the owner has added sets to in-session (>5) keeps the same two
// established sets rather than growing a third distinct weight. One set floors
// at [T].
//
// Validated by the owner's own log before being specified: Sep 8 push ran
...
// "E" is the established weight, "W" the target. The old WU (warm-up) and B
// (build) types are gone with the three-weight ramp — an established-weight set
// is productive volume, so it reads as its own thing rather than as a warm-up.
// `type` is never persisted (finish() writes only set/weight/reps/rpe/note and
// a resumed draft rebuilds plannedSets through buildRamp), so no old record
// carries a retired type.
